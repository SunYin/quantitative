...
- 做**行业吸引力**与**产业链**（例如 `quant industry AI` 看算力、存储、光电到应用；`quant industry 光电` / `长芯存储` 会落到对应层。长芯存储是未上市 IPO 样本，不能当代码打分）
- 点开个股看 **分时 / 1日 / 5日 / 日 K / 5年**：分时是 1 分钟折线，1 日是 5 分钟 K，日 K 仍是 1月–1年。短周期用 Yahoo 延迟行情并轮询，失败不编造分时。A/港红涨绿跌，美股绿涨红跌。走势不参与打分。
